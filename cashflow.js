...
/* ── Helper: fetch CSV rows as objects ── */
...
/* ── Next statement number for company (simple increment) ── */
...
/* ── Next statement number for contractor (per contractor) ── */
...
function ccfLoadRowForEdit(rowJson) {
    ...
    document.getElementById('ccf_statement_no').value = noVal;
...
function ccfBuildHistory(rows) {
    ...
    if (!rows || !rows.length) {
        list.innerHTML = '<div style="padding:12px 14px;text-align:center;color:rgba(255,255,255,0.35);font-size:12px;">لا توجد مستخلصات سابقة</div>';
    ...
    list.innerHTML = rows.slice().reverse().map((row, i) => {
        ...
        return `<div onclick="ccfLoadRowForEdit(${JSON.stringify(row).replace(/"/g,'&quot;')})"
            ...
            <span style="font-size:12px;font-weight:700;color:#5baddf;">${num}</span>
            <span style="font-size:11px;color:rgba(255,255,255,0.5);">${date}</span>
            <span style="font-size:12px;font-weight:700;color:#f5c842;">${parseFloat(String(amt).replace(/,/g,''))||amt}</span>
            <span style="font-size:10px;color:rgba(33,150,243,0.8);">تعديل ✎</span>
        </div>`;
    }).join('');
    panel.style.display = 'block';
}
...
async function ccfOpenHistory() {
    ...
    if (!_ccfRowsCache || !_ccfRowsCache.length) {
        list.innerHTML = '<div style="padding:12px 14px;text-align:center;color:rgba(255,255,255,0.4);font-size:12px;">⏳ جاري التحميل...</div>';
        panel.style.display = 'block';
        try {
            ...
            list.innerHTML = '<div style="padding:12px 14px;text-align:center;color:#ff8a80;">❌ تعذر التحميل</div>';
        }
    } else {
        if (panel.style.display === 'block') panel.style.display = 'none';
        else { ccfBuildHistory(_ccfRowsCache); panel.style.display = 'block'; }
    }
}
...
function concfLoadRowForEdit(rowJson) {
    ...
    if (noInp) noInp.value = row[noKey] || '';
...
function concfBuildHistory(rows, filterContractor) {
    ...
    list.innerHTML = displayRows.slice().reverse().map((row, i) => {
        ...
        return `<div onclick="concfLoadRowForEdit(${JSON.stringify(row).replace(/"/g,'&quot;')})"
            ...
            onmouseout="this.style.background='${bg}'">
            <span style="font-size:12px;font-weight:700;color:#f5c842;">${num}</span>
            ${!filterContractor && con ? `<span style="font-size:10px;color:rgba(255,255,255,0.5);">${con}</span>` : ''}
            <span style="font-size:11px;color:rgba(255,255,255,0.5);">${date}</span>
            <span style="font-size:12px;font-weight:700;color:#5cc890;">${parseFloat(String(tot).replace(/,/g,''))||tot}</span>
            <span style="font-size:10px;color:rgba(245,200,66,0.8);">تعديل ✎</span>
        </div>`;
    }).join('');
    panel.style.display = 'block';
}
